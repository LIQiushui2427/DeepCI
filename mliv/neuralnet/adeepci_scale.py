            ...
            for it, (x_1_a_j_batch, x_2_a_j_batch, x_1_a_k_batch, x_2_a_k_batch, 
                ...
                if (it % train_learner_every == 0):
                    ...
                    m = (x_1_a_j_batch - x_1_a_k_batch + x_1_b_k_batch - x_1_b_j_batch + \
                         pred_a_j - pred_a_k + pred_b_k - pred_b_j)*test*10000000000 #+ x_1_b_k_batch - x_1_b_j_batch \
                         #+ pred_a_j - pred_a_k + pred_b_k - pred_b_j)*test
                    D_loss = torch.maximum(torch.tensor(0),torch.mean(-1*(m)))  
                    print("learner loss", D_loss)
                ...
                if (it % train_adversary_every == 0):
                    ...
                    test = self.adversary(z_batch)**2
                    m = (x_1_a_j_batch - x_1_a_k_batch + x_1_b_k_batch - x_1_b_j_batch + \
                         pred_a_j - pred_a_k + pred_b_k - pred_b_j)*test*10000000000#(x_1_a_j_batch - x_1_a_k_batch + \
                           # x_1_b_k_batch - x_1_b_j_batch + \
                           #  pred_a_j - pred_a_k + \
                           #  pred_b_k - pred_b_j)*test
                    G_loss = - torch.maximum(torch.tensor(0),torch.mean(-1*(m))) + 100000*torch.mean(test)  # - torch.maximum(torch.tensor(0),torch.mean(-1*(m)))**2 #+ torch.mean(test)**2
                    print("adversary loss", G_loss)
